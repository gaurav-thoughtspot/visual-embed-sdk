...
You can add a custom action as a primary button or as a menu item in the *More* menu image:./images/icon-more-10px.png[the more options menu] on the *Answers* or *Pinboards* page. You can configure this custom menu item to initiate an action or callback to a parent application. 
...
//// 
[NOTE]
The data is available to the external application in the JSON (JavaScript Object Notation) format. You must parse the JSON data to get the values you need using JavaScript in the receiving application.
...
* xref:push-data-external-app.adoc#register-callback[Register the custom action in the SDK]
* xref:push-data-external-app.adoc#callback-initiate[Initiate a callback]
* xref:push-data-external-app.adoc#handle-data[Define classes and functions to handle the  custom action data]
...
[#register-callback]
=== Register the callback event through the Visual Embed SDK

Register for the callback in the Visual Embed SDK. This configuration is required to trigger an event when the custom action is initiated. 

The following example shows how to register for the `showData` callback in the `SearchEmbed` SDK.
...
For example, if the HTML page has an element with the ID as `show-data` (the callback ID), and it contains a table with the data triggered from the custom action, you can define the classes and functions to handle the details of this data as shown here:
...
----
[#handle-data]
==== Define functions and classes for handling custom action data
